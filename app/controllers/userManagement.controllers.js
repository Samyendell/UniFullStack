...
const login = (req, res) => {
    ...
    try {
        userManagementModel.getUserDetails(req.body.email, (err, userDetails) => {
            if (err) return res.status(500).send("Database error");

            const password_hash = getHash(req.body.password, userDetails.salt);

            if(password_hash != userDetails.password_hash) {
                return res.status(500).send("Incorrect password")
            }

            // look at the handling authenticeation pp it does it differently
    ...
    // incoming password is hashed using the salt from the db

    // the 2 passwords are compared
